...
#     train_label=pd.read_csv(X['label'])
    # train_beneficiary=pd.read_csv(X['beneficary'])[:1]
    # train_inpatient=pd.read_csv(X['inpatient'])[:1]
    # train_outpatient=pd.read_csv(X['outpatient'])[:1]
...
def predict():
    ...
    if request.method=='POST':
        ...
        path_bene= 'C:\\Users\\Nimble\\Downloads\\CS_1\\'+file_bene.filename
        path_in='C:\\Users\\Nimble\\Downloads\\CS_1\\'+file_inp.filename
        path_out='C:\\Users\\Nimble\\Downloads\\CS_1\\'+file_out.filename

        # path_bene = "./"+file_bene.filename
        # path_in="./"+file_inp.filename
        # path_out="./"+file_out.filename
        # file_inp.save(path_bene)
        # file_inp.save(path_in)
        # file_out.save(path_out)

        print(path_out)
        # # benefi = request.files.get('beneficiary')
        # # inpa = request.files.get('inpatient')
        # # out = request.files.get('outpatient')

        # print(path_in)
        df,id_=final_pipeline(temp_bene,temp_inp,temp_out)
        count=0
        if int(df['Prediction'])==1:
            string='Provider_ID :='+str(str(id_))+'  is FRAUD'
        
        else:
            string='Provider_ID :='+str(str(id_))+'  is NOT FRAUD'
